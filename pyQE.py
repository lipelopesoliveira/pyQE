...
kpath_dict = {
    '0': [r'-$\frac{\pi}{a}$', r'$\Gamma$', r'-$\frac{\pi}{a}$'],
    '1': [r'$\Gamma$', 'X', 'M', r'$\Gamma$', 'R', 'X'],
    '2': ['L', r'$\Gamma$', 'X', 'W', r'$\Gamma$'],
    '3': [r'$\Gamma$', 'H', 'N', r'$\Gamma$', 'P', 'H'],
    '4': [r'$\Gamma$', 'M', 'K', r'$\Gamma$', 'A', 'L', 'H', 'A'],
    '5': ['Trigonal R, 3fold axis c '],
    '-5': [' Trigonal R, 3fold axis'],
    '6': ['G', 'X', 'M', 'G', 'Z', 'R', 'A', 'Z'],
    '7': ['G', 'X', 'P', 'N', 'G', 'Z', 'S', r'S$_0$', r'$\Gamma$'],
    '8': ['G', 'X', 'H', 'C', 'G', 'Y', 'H1', 'G'],
...
class Build_Files(QMainWindow):
    ...
    def open_gv_file(self, prefix):
        ...
        if os.path.exists(os.path.join(self.path, prefix + '.gjf')) is True:
            gv_file = os.path.join(self.path, prefix + '.gjf')

        else:
            print("Cannot find GaussView file! Please select the file: ")
            gv_file = QFileDialog.getOpenFileName(
                self, 'Open .gjf file', self.path, "GaussView Job File (*.gjf)")[0]
        ...
        for i in range(len(pos)):
            ...
            if (pos[i][0] in list_type) is False:
                list_type += pos[i][0]
    ...
    def read_qe_in(self, path, prefix):

        if os.path.exists(os.path.join(path, prefix+'.scf.in')) is True:
            qe_file = os.path.join(path, prefix+'.scf.in')

        else:
            print("Cannot find input file! Please select the file: ")
            qe_file = QFileDialog.getOpenFileName(self, 'Open Quantum ESPRESSO file', self.path,
                                                  "QE SCF file (*.scf.in);; QE IN file (*.in) ;;All Files (*.*)")[0]
        ...
        for i in range(len(file)):
            if 'ATOMIC_POSITIONS' in file[i]:
                ...
                atomic_pos = [list(filter(lambda a: a != '', x))
                              for x in atoms]
    ...
    def read_vcrelax_out(self):
        ...
        if os.path.exists(os.path.join(self.path, self.prefix+'.vcrelax.out')) is True:
            qe_file = os.path.join(self.path, self.prefix+'.vcrelax.out')

        else:
            print("Cannot find vcrelax file! Please select the file: ")
            qe_file = QFileDialog.getOpenFileName(self, 'Open Quantum ESPRESSO file', self.path,
                                                  "QE VC-Relax file (*.vcrelax.out);; QE VC-Relax file (*.vc-relax.out) ;;All Files (*.*)")[0]
        ...
        for i in file:
            if '!' in i:
                energy = np.append(energy, [float(i[32:-3])])
        data['energy'] = energy

        force = np.array([])
        for i in file:
            if 'Total force' in i:
                force = np.append(force, [float(i[21:35])])
        data['force'] = force

        total_stress = np.array([])
        for i in file:
            if ' total   stress' in i:
                total_stress = np.append(total_stress, [float(i[71:-1])])
        ...
        for i in range(len(file)):
            ...
            if 'Begin final coordinates' in file[i]:
                pos_begin = i
            if 'End final coordinates' in file[i]:
                pos_end = i
...
class Bulk_Modulus(QMainWindow):
    ...
    def get_bulk_data(self):
        ...
        for i in file_names:
            ...
            for line in tmp:
                if 'unit-cell volume' in line:
                    ...
                    alat_list += [float(line[33:].rstrip('a.u.\n'))]
        ...
        # Convert alat from [a.u.] to angstrom
        alat_list = np.array(alat_list)*0.529177

        # Save data as a comma separetad value in the current working directory
        save = open(os.path.join(os.getcwd(), self.prefix + '.csv'), 'w')

        save.write('ev/atom;A^3/atom;A\n')

        for i in range(len(cell_vol_list)):
            save.write(str(
                cell_vol_list[i]) + ';' + str(cell_energy_list[i]) + ';' + str(alat_list[i]) + '\n')
    ...
    def generate_bulk_data(self):
        ...
        dir_list = [f for f in os.listdir(
            os.getcwd()) if os.path.isdir(f) is True]
        ...
        for structure in dir_list:
            ...
            n_atoms = 1
            for i in file_names:
                tmp = open(os.path.join(os.path.join(
                    os.getcwd(), structure), i)).readlines()
                for line in tmp:
                    if 'unit-cell volume' in line:
                        cell_vol_list += [float(line[33:].rstrip('(a.u.)^3\n'))]
                    elif 'lattice parameter (alat)' in line:
                        alat_list += [float(line[33:].rstrip('(a.u.)^3\n'))]
                    elif 'number of atoms/cell' in line:
                        n_atoms = int(line[33:].rstrip('\n'))
                    elif '!' in line:
                        cell_energy_list += [float(line[33:].rstrip('Ry\n'))]
            ...
            for i in range(len(cell_vol_list)):
                save.write(str(cell_vol_list[i]) +
                           ';' + str(cell_energy_list[i])+'\n')
    ...
    def plot_bulk(self):
        ...
        for name in files_list:
            tmp = np.transpose(np.loadtxt(
                os.path.join(self.path, name), delimiter=';'))
        ...
        for key in data:
            ...
            plt.plot(data[key][0], data[key][1] + 11.84567612*13.605698066,
                     'o', ms=6, lw=1.5, alpha=1, mfc=cor, color='black', label=key)
        ...
        plt.savefig(os.path.join(self.path, 'bulk.pdf'), dpi=150,
                    transparent=False, figsize=(16, 16), format='pdf')
        plt.show()
    ...
    def get_elastic_matrix(self):
        ...
        tmp = open(os.path.join(self.path, self.prefix +
                   '.elastic.out')).readlines()
        ...
        for i in range(len(tmp)):
            if 'Elastic constants C_ij (kbar) ' in tmp[i]:
                tensor[0] = np.array(
                    self.float_string(tmp[i+2][7:].rstrip('\n')))
                tensor[1] = np.array(
                    self.float_string(tmp[i+3][7:].rstrip('\n')))
                tensor[2] = np.array(
                    self.float_string(tmp[i+4][7:].rstrip('\n')))
                tensor[3] = np.array(
                    self.float_string(tmp[i+5][7:].rstrip('\n')))
                tensor[4] = np.array(
                    self.float_string(tmp[i+6][7:].rstrip('\n')))
                tensor[5] = np.array(
                    self.float_string(tmp[i+7][7:].rstrip('\n')))
...
class Bands_DOS(QMainWindow):
    ...
    def Symmetries(self):
        """
        Obtem as coordenadas dos pontos de alta simetria em 
        uma dimensao do arquivo prefix.band.out.

        Parameters
        ----------
        prefix : str
            O prefixo do cálculo

        Returns
        -------
        array
            Um array do numpy contendo as coordenadas em 1D dos pontos de alta simetria. 
        """
        if os.path.exists(os.path.join(self.path, self.prefix+'.band.out')) is True:
            qe_file = os.path.join(self.path, self.prefix+'.band.out')

        else:
            print("Cannot find .band file! Please manually select the file:")
            qe_file = QFileDialog.getOpenFileName(self,
                                                  'Open Quantum ESPRESSO file',
                                                  self.path,
                                                  "QE Band file (*.band.out);; All Files (*.*)")[0]
        temp = open(qe_file, 'r')
        x = np.zeros(0)
        for i in temp:
    ...
    def get_Fermi(self):
        ...
        if os.path.exists(os.path.join(self.path, self.prefix+'.nscf.out')) is True:

            qe_file = os.path.join(self.path, self.prefix+'.nscf.out')
        else:

            if os.path.exists(os.path.join(self.path, self.prefix+'.scf.out')) is True:

                qe_file = os.path.join(self.path, self.prefix+'.scf.out')

            else:
                print("Cannot find any scf or nscf out file! Please select the one: ")
                qe_file = QFileDialog.getOpenFileName(self, 'Open Quantum ESPRESSO file',
                                                      self.path, "SCF OUT file (*.scf.out);; NSCF OUT file (*.nscf.out);; All Files (*.*)")[0]
    ...
    def get_bands(self):
        ...
        if os.path.exists(os.path.join(self.path, self.prefix+'.band.gnu')) is True:
            qe_file = os.path.join(self.path, self.prefix+'.band.gnu')

        else:
            print("Cannot find band file! Please select the file: ")
            qe_file = QFileDialog.getOpenFileName(
                self, 'Open Quantum ESPRESSO file', self.path, "QE Band file (*.band.gnu);; All Files (*.*)")[0]
    ...
    def getDOS(self):
        ...
        if os.path.exists(os.path.join(self.path, self.prefix+'.dos')) is True:
            qe_file = os.path.join(self.path, self.prefix+'.dos')

        else:
            print("Cannot find band file! Please select the file: ")
            qe_file = QFileDialog.getOpenFileName(
                self, 'Open Quantum ESPRESSO file', self.path, "QE DOS file (*.dos);; All Files (*.*)")[0]
    ...
    def get_all_pdos(self):
        ...
        list_atoms = Build_Files.read_qe_in(
            self.prefix, self.path, self.prefix)['list_atom']
        ...
        for i in range(len(list_atoms)):
            for j in range(len(self.orbitals)):
                if list_atoms[i][0] == self.orbitals[j][0]:
                    list_atoms[i].append(self.orbitals[j][-1])
        # Itera sobre todos os átomos para pegar seu índice e tipo

        for i in range(len(list_atoms)):
            file = self.prefix+".pdos.pdos_atm#" + \
                str(i+1)+"("+list_atoms[i][0]+")_wfc#"
        # itera sobre os tipos de orbitais (1s ou 2p) e os adiciona em list_dos_s ou list_dos_p
            for j in range(len(list_atoms[i][-1])):
                ...
                # Cria a lista das energias
                energy.append(self.get_pdos_file(filej)[0])
                # Cria a lista dos orbitais 1s
                if list_atoms[i][-1][j] == '1(s)':
                    list_dos_s[i] = self.get_pdos_file(
                        file+list_atoms[i][-1][j])[1:]

                # Cria a lista dos orbiais 2p
                if list_atoms[i][-1][j] == '2(p)':
                    list_dos_p += [self.get_pdos_file(
                        file+list_atoms[i][-1][j])[1:]]

        energy = np.array(energy[0]).astype(float)

        energy = energy - self.eFermi[0]
        return energy, list_dos_s, list_dos_p

    def get_pdos_byatom(self):
...
class Bands_Plot(QMainWindow):
    ...
    def plotband(self, eFermi=True, y=None, figname=False):
        ...
        if y is None:
            for i in range(len(energy)):
                if max(energy[i]) > ymax:
                    ymax = max(energy[i])
                if min(energy[i]) < ymin:
                    ymin = min(energy[i])

        plt.figure(dpi=170)
        for i in range(len(energy)):
            plt.plot(k, energy[i], lw=0.55)
        plt.plot([kpoints[1:-1], kpoints[1:-1]], [ymin, ymax],
                 '--', lw=0.55, color='red', alpha=0.75)
        plt.axis([min(k), max(k), ymin, ymax])
        if eFermi is True:
            plt.plot([min(k), max(k)], [0, 0], '-',
                     lw=0.55, color='red', alpha=0.75)
    ...
    def pltbandasDOS(self, eFermi=True, y=[-10, 10], figname=True, line=True):
        ...
        ax1.plot([kpoints[1:-1], kpoints[1:-1]],
                 [ymin, ymax], '--', lw=0.55, color='red')
        ax1.axis([min(k), max(k), ymin, ymax])
        ax1.plot([min(k), max(k)], [0, 0], '-',
                 lw=0.55, color='red', alpha=0.75)
        ...
        ax2.plot([min(dose)[0], max(dose)[0]*1.1], [0, 0],
                 '-', lw=0.55, color='red', alpha=0.75)
        ...
        if figname is not False:
            plt.savefig(os.path.join(self.Bands_D.path, self.prefix +
                        '.bands.pdf'), dpi=150, transparent=False)
        plt.show()

    def pltbandaspDOS(self, eFermi=True, y=[-2, 2], figname=True, line=True):
        ...
        plt.fill_between(pdos_total, 0, energy, alpha=.4,
                         color='gray', linewidth=0.0)
        plt.fill_between(pdos_p[1], 0, energy, alpha=.4,
                         color='r', linewidth=0.0)
        plt.fill_between(pdos_s, 0, energy, alpha=.4, color='b', linewidth=0.0)

        ax2.set_ylim(ax1.get_ylim())
        ax2.set_xlim(min(dose)[0], max(dose)[0]*0.45)
        ax2.plot([min(dose)[0], max(dose)[0]*.8], [0, 0],
                 '-', lw=0.55, color='red', alpha=0.75)
        # ax2.fill(dose,energy, color='green',alpha=0.7)
        ax2.set_yticks(())
    ...
    def pltbandas_all_pDOS(self, eFermi=True, y=[-2, 2], figname=True, line=True):
        ...
        if eFermi is False:
            k, energy = self.Bands_D.get_bands()
        kpoints = self.Bands_D.Symmetries()
        print('High simmetry points: ', kpoints)
        print('High simmetry labels: ', k_path_label)
        ...
        for i in range(len(energy)):
            if line is True:
                ...
                ax1.plot(k, energy[i], '.',  ms=1, lw=0.6, color='black')
        ax1.plot([kpoints[0:-1], kpoints[0:-1]],
                 [ymin, ymax], '--', lw=0.55, color='red')
        ax1.axis([min(k), max(k), ymin, ymax])
        ax1.plot([min(k), max(k)], [0, 0], '-',
                 lw=0.55, color='red', alpha=0.75)
        ...
        ax2.fill_between(pdos_s, 0, energy, alpha=ap,
                         color='blue', linewidth=0.0)
        ax2.fill_between(pdos_p[2], 0, energy, alpha=ap,
                         color='red', linewidth=0.0)
        ax2.fill_between(pdos_p[3], 0, energy, alpha=ap,
                         color='green', linewidth=0.0)
        ax2.fill_between(pdos_p[4], 0, energy, alpha=ap,
                         color='yellow', linewidth=0.0)
        ax2.fill_between(pdos_total, 0, energy, alpha=ap,
                         color='gray', linewidth=0.0)

        ax2.set_ylim(ax1.get_ylim())
        ax2.set_xlim(0, 3)
        ax2.plot([0, 4], [0, 0], '-', lw=0.55, color='red', alpha=0.75)
        ...
        plt.savefig(os.path.join(self.Bands_D.path, self.prefix +
                    '.bands.pdf'), dpi=150, transparent=False)
        plt.savefig(os.path.join(self.Bands_D.path, self.prefix +
                    '.bands.svg'), dpi=150, transparent=True)
        plt.savefig(os.path.join(self.Bands_D.path, self.prefix +
                    '.bands.png'), dpi=350, transparent=False)
        plt.show()
    ...
    def plot_pdos(self, x=None, y=None, sump=True, figname=True):
        ...
        if sump is True:
            plt.plot(energy, pdos_p[1], label="pDOS Orbital P", color='r')
        if sump is True:
        ...
        if sump is not True:
            ...
            plt.plot(energy, pdos_p[4],
                     label="pDOS Orbital Pz", color='yellow')
            plt.plot(energy, pdos_s, label="pDOS Orbital S", color='b')

            plt.fill_between(
                energy, 0, pdos_p[2], alpha=ap, color='r', linewidth=0.0)
            plt.fill_between(
                energy, 0, pdos_p[3], alpha=ap, color='green', linewidth=0.0)
            plt.fill_between(
                energy, 0, pdos_p[4], alpha=ap, color='yellow', linewidth=0.0)
            plt.fill_between(energy, 0, pdos_total, alpha=ap,
                             color='gray', linewidth=0.0)
            plt.fill_between(energy, 0, pdos_s, alpha=ap,
                             color='gray', linewidth=0.0)
        ...
        if sump is True:
            plt.fill_between(energy, 0, pdos_total, alpha=ap,
                             color='gray', linewidth=0.0)
            plt.fill_between(
                energy, 0, pdos_p[1], alpha=ap, color='r', linewidth=0.0)
            plt.fill_between(energy, 0, pdos_s, alpha=ap,
                             color='b', linewidth=0.0)

        plt.plot([0, 0], [0, max(pdos_total)], '--', color='red')

        plt.legend(loc='upper right', framealpha=0, prop={'size': 12.5})
        plt.tight_layout()

        if x is not None:
            plt.axis([x[0], x[1], 0, float(max(pdos_s))])
        if y is not None:
            plt.axis([x[0], x[1], 0, y])
        if x is None and y is None:
            plt.axis([float(min(energy)), float(
                max(energy)), 0, float(max(pdos_total))])

        plt.xlabel('Energy (E-$\epsilon_f$) [eV]')
        plt.ylabel('DOS($\epsilon$) [states/eV/unit. cell]')
        plt.tight_layout()

        if figname is not False:
            plt.savefig(os.path.join(self.Bands_D.path, self.prefix +
                        '.pdos.pdf'), dpi=150, transparent=False)
            plt.savefig(os.path.join(self.Bands_D.path, self.prefix +
                        '.pdos.svg'), dpi=150, transparent=False)
            plt.savefig(os.path.join(self.Bands_D.path, self.prefix +
                        '.pdos.png'), dpi=150, transparent=False)
...
class Plot_Phonon(QMainWindow):
    ...
    def __init__(self, prefix):
        ...
        self.kpath_dict = {'0': [''],
                           ...
                           '5': ['Trigonal R, 3fold axis c '],
                           '-5': [' Trigonal R, 3fold axis'],
                           '6': [r'$\Gamma$', 'X', 'M', r'$\Gamma$', 'Z', 'R', 'A', 'Z'],
                           '7': [r'$\Gamma$', 'X', 'P', 'N', r'$\Gamma$', 'Z', 'S', r'S$_0$', r'$\Gamma$'],
                           '8': [r'$\Gamma$', 'X', 'H', 'C', r'$\Gamma$', 'Y', 'H1', r'$\Gamma$', 'Orthorhombic P  '],
                           '9': ['  Orthorhombic base-centered(bco)'],
                           '-9': ['as 9, alternate description'],
                           '91': [' Orthorhombic one-face base-centered A-type'],
                           '10': ['Orthorhombic face-centered'],
    ...
    def get_sym(self):
        ...
        if os.path.exists(os.path.join(self.path, self.prefix+'.plotband.out')) is True:
            print('File ', self.prefix+'.plotband.out', ' found!')
            qe_file = os.path.join(self.path, self.prefix+'.plotband.out')

        else:
            print("Cannot find ", self.prefix+'.plotband.out',
                  " file! Please select the file: ")
            qe_file = QFileDialog.getOpenFileName(
                self, 'Open Quantum ESPRESSO file', self.path, "QE Band file (*.plotband.out);; All Files (*.*)")[0]
    ...
    def get_ibrav(self):
        ...
        if os.path.exists(os.path.join(self.path, self.prefix+'.scf.in')) is True:
            print('File ', self.prefix+'.scf.in', ' found!')
            qe_file = os.path.join(self.path, self.prefix+'.scf.in')

        else:
            print("Cannot find ", self.prefix+'.scf.in',
                  " file! Please select the file: ")
            qe_file = QFileDialog.getOpenFileName(
                self, 'Open Quantum ESPRESSO file', self.path, "QE PW in file (*.scf.in);; All Files (*.*)")[0]

        in_file = open(qe_file, 'r').readlines()

        for line in in_file:
            if ' ibrav' in line:
                self.ibrav = line.rstrip(',\n').split()[-1]
    ...
    def get_kPath(self):
        ...
        self.get_ibrav()
        if self.ibrav:
            self.kpath = self.kpath_dict[self.ibrav]

    def get_freq(self):

        if os.path.exists(os.path.join(self.path, 'freq.plot')) is True:
            print('File ', self.prefix+'.freq.plot', ' found!')
            qe_file = os.path.join(self.path, 'freq.plot')

        else:
            print("Cannot find", self.prefix+'.freq.plot',
                  " file! Please select the file: ")
            qe_file = QFileDialog.getOpenFileName(
                self, 'Open Quantum ESPRESSO file', self.path, "QE Plot file (*.plot);; All Files (*.*)")[0]
    ...
    def get_DOS(self):
        ...
        if os.path.exists(os.path.join(self.path, self.prefix+'.phdos')) is True:
            print('File ', self.prefix+'.phdos', ' found!')
            qe_file = os.path.join(self.path, self.prefix+'.phdos')

        else:
            print("Cannot find ", self.prefix+'.phdos',
                  " file! Please select the file: ")
            qe_file = QFileDialog.getOpenFileName(
                self, 'Open Quantum ESPRESSO file', self.path, "QE PHDos file (*.phdos);; All Files (*.*)")[0]
    ...
    def pltbandasDOS(self, figname=True, line=True):

        k, energy = self.get_freq()
        self.get_kPath()
        kpoints = self.get_sym()

        ymin, ymax = np.min(energy) - 100, np.max(energy)*1.1
        ymin, ymax = np.min(energy) - 100, 200
        ...
        for i in range(len(energy)):
            if line is True:
                ax1.plot(k, energy[i], lw=1., color='black')
            else:
                ax1.plot(k, energy[i], '.', ms=1, lw=0.7, color='black')

        ax1.plot([kpoints[1:-1], kpoints[1:-1]],
                 [ymin, ymax], lw=0.6, color='red')
        ax1.axis([min(k), max(k), ymin, ymax])
        ax1.plot([min(k), max(k)], [0, 0], '--', lw=.6, color='red')

        if os.path.exists(os.path.join(self.path, 'Exp.csv')) is True:
            print('File ', 'Exp.csv', ' found!')
            exp = np.transpose(np.genfromtxt(
                str(self.path) + '/Exp.csv', delimiter=';'))
            ax1.plot(exp[0], exp[2], '.',  ms=8,
                     lw=2, mfc='red', color='black')

        ax1.set_ylabel(r'Frequency [cm\textsuperscript{-1}]')

        ax1.set_xlabel('K-path')
        ax1.set_xticks(kpoints)
        ax1.set_xticklabels(self.kpath[0:len(kpoints)])
        ...
        plt.tight_layout()
        if figname is not False:
            plt.savefig(os.path.join(self.path, self.prefix +
                        '_phdisp.pdf'), dpi=150, transparent=False)
            plt.savefig(os.path.join(self.path, self.prefix +
                        '_phdisp.svg'), dpi=150, transparent=True)
            plt.savefig(os.path.join(self.path, self.prefix +
                        '_phdisp.png'), dpi=150, transparent=False)
...
header = '''
-----------------------------------------------------------------------------------
            pyQE - A front-end suite for Quantum ESPRESSO data analysis
                    Code created by Felipe Lopes
-----------------------------------------------------------------------------------
'''
...
- Electronic structure
   10 - Plot band structure 
   11 - PLot band structure and DOS
   12 - Plot band structure and summed pDOS
   13 - Plot band structure and all pDOS
   14 - plot total DOS
   15 - Plot pDOS
3 - Plot phonon dispersion
4 - Plot Bulk modulus information
5 - Elastic
'''
print(menu)
